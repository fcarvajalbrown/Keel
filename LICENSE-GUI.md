...
- **The engine** — the Python library and CLI (`keel.py`, `recipes.py`,
  `mixer.py`, `mastering.py`, `meters.py`, `build.py`) — is **GNU AGPL-3.0**.
  See [`LICENSE`](LICENSE). It is free and open source.
- **The desktop GUI** — `gui.py`, `userpresets.py`, and the packaged
  `Keel.exe` / `Keel.app` application built from them — is licensed under the
  **PolyForm Noncommercial License 1.0.0** (reproduced below), **plus the
  additional free-use grant** in the next section, **and** with a separate
...
As an exception that **extends** the PolyForm Noncommercial grant below, the
licensor (Felipe Carvajal Brown) also permits, free of charge:

- **Individual creators** using the Keel GUI to mix and master **their own
  musical works** — including selling, streaming, distributing, or otherwise
  monetizing those works. Your songs are yours; Keel takes no rights in your
